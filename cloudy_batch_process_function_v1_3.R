fu <- function(location=choose.dir(),
               ...
               ds = readline(prompt=paste("What is the droplet volume (nL; default = 0.797 nL)?",
"",
"Biorad PCR MM no DUTP and Residual DNA Quantitation MM - 0.771 nL", 
"Biorad PCR Mix with DUTP - 0.834 nL (F-D-0512 v10)", sep = "\n")),
               ds2 = as.numeric(unlist(ds)),
               dVol = ifelse(!is.na(ds2),ds2,  0.797), 
               channel = menu(c("Channel 1", "Channel 2"), title="Which channel?"),
               sV = readline(paste("What is the sample volume (\u00B5L; default = 5 \u00B5L)?",
"",
"i.e. volume of sample loaded into a well without the excess volume calculated for the pipetting", sep =  "\n")),
